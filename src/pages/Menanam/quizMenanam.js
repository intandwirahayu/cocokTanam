...
class QuizMenanam extends Component {
    _isMounted = false;
    ...
    //sudah menjalankan rencana menanam
    checkRiwayatMenanam = async () => {
        try{
            //mencari data riwayat menanam berdasarkan id user
            ...
            .then(snapshot => {
                ...
                if(valueField.includes(keyData)){
                    this.deleteRencanaRunMenanam1(keyData);
                }else{
                    this.deleteRencanaRunMenanam(keyData);
                }
            })
            .catch(() => {
                this.deleteRencanaRunMenanam(keyData);
    ...
    deleteRencanaRunMenanam1 = async (keyData) => {
        try{
            //menghapus salah satu isi di data rencana menanam
            //tidak menambah data riwayat menanam.
    ...
    deleteRencanaRunMenanam = async (keyData) => {
        try{
            //menghapus salah satu isi di data rencana menanam
            ...
            .then(snapshot => {
                this.searchIdRiwayatMenanam(keyData);
            })
        }catch(e){
            console.log(e);
        }
    ...
    searchIdRiwayatMenanam = async (keyData) => {
        try{
            ...
            .get()
            .then(snapshot => {
                let dataIdRiwayatMenanam = [];

                snapshot.forEach(documentSnapShot => {
                    dataIdRiwayatMenanam.push(documentSnapShot.id);
                });

                const {idUser} = this.state;

                if(dataIdRiwayatMenanam.includes(idUser)){
                    this.searchFieldRiwayatMenanam(idUser, keyData);
                }else{
                    this.makeRiwayatMenanam(idUser, keyData);
                }
            });
        }catch(e){
            console.log(e);
    ...
    searchFieldRiwayatMenanam = async (idUser, keyData) => {
        try{
            //mencari nilai field terakhir di data riwayat menanam berdasarkan id user
            await firestore()
            .collection('data_riwayat_menanam')
            .doc(idUser)
            .get()
            .then(snapshot => {
                ...
                if(countDataKey > 0){
                    this.addRiwayatMenanam(keyField, idUser, keyData);
                }else{
                    this.makeRiwayatMenanam(idUser, keyData);
                }
    ...
    addRiwayatMenanam = async (keyField, idUser, keyData) => {
        try{
            //menambah data riwayat menanam dengan key field yang sudah ditentukan
    ...
    makeRiwayatMenanam = async (idUser, keyData) => {
        try{
